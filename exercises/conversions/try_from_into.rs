...
// Also note, that chunk of correct rgb color must be integer in range 0..=255.
    ...
    fn convert(red: i16, green: i16, blue: i16) -> Result<Color, String> {
         u8::try_from(red)
            .and_then(|red| u8::try_from(green)
                .and_then(|green| u8::try_from(blue)
                    .map(|blue|Color{red,green,blue})
                )
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        if slice.len() != 3 { Err("prout".to_string()) }
        else { convert(slice[0], slice[1], slice[2]) }
    }
}
